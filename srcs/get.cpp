...
/*   Updated: 2024/06/06 15:51:44 by brolivei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void processRequest(const std::string& request, ServerInfo& server)
{
	...
	if (parser.parseRequest(requestCopy, requestMsg, maxSize))
	{
		...
		//{
			...
			std::cout << serverConfig.Path_CGI << std::endl;
			for (std::map<std::string, conf_File_Info>::iterator it = serverConfig.LocationsMap.begin(); it != serverConfig.LocationsMap.end(); it++)
			{
				std::cout << std::endl << it->first << " -> ";
				for (std::set<std::string>::iterator setIt = it->second.allowedMethods.begin(); setIt != it->second.allowedMethods.end(); setIt++)
					std::cout << *setIt << " ";
			}
